...
class ClimaSemanal:
    ...
    def ingresar_temperaturas(self):
        ...
        for i in range(7):
            while True:
                try:
                    temp = float(input(f"Día {i + 1}: "))  # Pedir la temperatura por día
    ...
    def calcular_promedio(self):
        ...
        return suma / len(self.__clima_semanal)  # Promedio de las temperaturas
...
def main():
    ...
    clima_semanal.ingresar_temperaturas()  # Ingresar las temperaturas de la semana
    promedio = clima_semanal.calcular_promedio()  # Calcular el promedio
    print(f"\nEl promedio semanal de temperaturas es: {promedio:.2f}°C")  # Mostrar el promedio
